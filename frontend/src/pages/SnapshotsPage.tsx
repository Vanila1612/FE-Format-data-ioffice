...
export function SnapshotsPage() {
  ...
  const visibleSnapshotRows = report.data ? filterResultBoardRows(report.data.boardRows, boardSearch, unitScope) : [];
  const visibleSnapshotTotals = totalsFromBoardRows(visibleSnapshotRows);
  return <section className="page-stack">
    ...
    {selectedId && <div className="panel"><div className="panel-head"><div><h2>Kết quả đã chốt</h2><p>{reportPeriod(selectedSnapshot)} · Dữ liệu đã lưu, không bị thay đổi khi văn bản hoặc quy tắc hiện tại thay đổi.</p></div></div>{report.isLoading ? <LoadingState /> : report.isError ? <ErrorState message={report.error.message} retry={() => report.refetch()} /> : <><div className="kpi-grid"><Kpi label="Tổng văn bản" value={visibleSnapshotTotals.total} /><Kpi label="Đã ký số" value={visibleSnapshotTotals.signed} /><Kpi label="Chưa ký số" value={visibleSnapshotTotals.unsigned} /><Kpi label="Tỷ lệ ký" value={`${visibleSnapshotTotals.signRate}%`} /></div><ResultBoardTable rows={report.data!.boardRows} search={boardSearch} onSearchChange={setBoardSearch} scope={unitScope} onScopeChange={setUnitScope} /></>}</div>}
  </section>;
}
